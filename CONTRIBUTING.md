...
| TYPE | 使うとき | 例 |
|------|---------|-----|
| `feat` | 新機能の追加 | `feat: 出航／停泊ボタンと経過時間計算を追加` |
| `fix` | バグ修正 | `fix: 無制限モードで進捗が101%になる不具合を修正` |
| `style` | 見た目・デザインの変更 | `style: 海図の天候オーバーレイを調整` |
| `refactor` | 動作を変えないコード整理 | `refactor: useVoyagesフックを整理` |
...
| 項目 | 形式 | 例 |
|------|------|----|
| ブランチ名 | `TYPE/english-brief-description` | `feat/departure-timer` |
| PRタイトル | ブランチ名と同じ | `feat/departure-timer` |
...
# 2. 作業ブランチを作成して移動する
# 例: git switch -c feat/departure-timer
...
git add .
git commit -m "feat: 出航／停泊ボタンと経過時間計算を追加"

# 5. リモートにプッシュする
# 例: git push origin feat/departure-timer
...
# 9. 作業ブランチを削除する
# 例: git branch -d feat/departure-timer
...
- `.env*` / `dist/` / `out/` / `node_modules/` は絶対にコミットしないこと（→ `docs/constraints.md` 参照）
- `main` ブランチへの直接コミット・プッシュは禁止。必ずブランチを作成しPRを経由すること
  （唯一の例外: 空リポジトリにmainブランチを作るための初回コミット・プッシュ。`quickstart.md` STEP 3-2の手順に限る）
