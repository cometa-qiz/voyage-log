...
| 機能 | 送りの理由 |
|------|-----------|
| ポモドーロモード（25分作業／5分休憩、休憩中は航行時間停止） | v1.5で実装予定。正式仕様は9章を参照 |
...
- 作業／休憩の時間はデフォルト25分／5分。ユーザーが設定で変更可能
  （localStorageに保存、全航路共通の設定値）
- 出航中の航路にのみ「⏱️ ポモドーロ」ボタンが表示され、開始/停止を
  トグルする（プロトタイプは🍅アイコンだが、本実装では⏱️に変更する）
- 状態は `Voyage.pomo: { phase: 'work' | 'break', phaseStart: number } | null`
  としてFirestoreに保存する。書き込みは「開始」「フェーズ切り替え」
  「手動停止」「停泊」のタイミングのみ（毎秒書き込みはしない）
- 表示中のカウントダウンはphaseStartから`now`との差分をクライアント側で
  毎秒計算する（sailStartベースの航行時間計算と同じ方式）
- 複数の航路を同時に出航させている場合、ポモドーロの進行は**現在表示中の
  航路のみ**。他の航路は表示を切り替えた時点で自動的に追いつく
  （プロトタイプは全航路を常時バックグラウンド進行させるが、v1.5では
  単純化する）
- 休憩中は航行時間（進捗）が止まる。`sessBreakMs`（現セッションの
  累積休憩時間）を航行時間の計算から差し引くことで実現する
- 停泊すると`pomo`・`sessBreakMs`とも自動的にリセットされる
- 効果音: 作業開始時・休憩開始時にそれぞれ専用の音を鳴らす
